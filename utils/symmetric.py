...
class symmetric:
    ...
    def load_symmetric_key(self, key_txt: str, nonce_txt: str) -> None:
        ...
        try:
            ...
            logging.warning(f" Symmetric key was not loaded from file {key_txt}\n{err}")

        try:
            with open(nonce_txt, "rb") as nonce_file:
                self.__nonce = nonce_file.read()
            logging.info(f" Extra parameter successfully loaded from {nonce_txt}")

        except OSError as err:
            logging.warning(f" Symmetric key was not loaded from file {nonce_txt}\n{err}")
